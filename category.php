...
<body style="overflow-x:hidden">
        ...
        <div class="sidebar pe-4 pb-3">
            ...
            <a href="home" class="navbar-brand mx-4 mb-3" style="display: block;text-align: center;width: 100%;">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <?php } ?>
                    <?php if($role=='Admin'){ ?>
                    <a href="category" class="nav-item nav-link active"><i class="fa fa-th me-2"></i>Category's</a>
                    <?php } ?>
                    <?php if($role=='User'){ ?>
    ...
    <div id="full" style="position:fixed;top:0;left:0;width:100%;height:100%;background:red;    background: rgba(0,0,0,.9);z-index: 9999;display:none;overflow:auto">
            <div class="row" style="margin-top:20px">
                <div class="offset-md-2 col-md-8">
                    <div class="row" style="margin-bottom:25px">
                        <div class="offset-1 col-10">
                        <h2 style="text-align:center;border:2px solid">Add category:</h2>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <input type="text" name="domain" id="domain_add" placeholder="Domaine" class="form-control">
                        </div>
                    </div>
                    <div class="form-btn text-center">
                            <button class="btn btn-success" onclick='addc()' type="button">Add category</button>
                            <p id="form-message"></p>
                        </div>
                </div>
            </div>
    ...
    <script>
        ...
        function user()
        {
            document.getElementById('users').innerHTML="<tr><th>Id</th><th>Domain</th><th>Created at</th><th></th></tr> ";
            $.post('controller', {service:'category', type:'<?php echo $role; ?>'}).done(function(response){
                console.log(response)
            obj = JSON.parse(response);
            for (let i = 0; i < obj.length; i++) {
                var rol="User";
                
                document.getElementById('users').innerHTML+='<tr style="height:70px"><th scope="row">'+obj[i].id+'</th> <td class="tbname">'+obj[i].domain+'</td><td class="tbcreated">'+obj[i].created+'</td><td><i class="fa fa-edit" style="color:brown;cursor:pointer" onclick="edit_us('+obj[i].id+')" aria-hidden="true"></i><i class="fa fa-trash" style="color:red;cursor:pointer" onclick="delete_us('+obj[i].id+')" aria-hidden="true"></i></td></tr>'
            }
        //     var A = document.getElementsByClassName('tbdomaine');
        //     for (let i = 0; i < A.length; i++) {
        //         if(i>=10)
        ...
        function addc()
        {
            domaine = document.getElementById('domain_add').value.trim();
            $.post('controller', {service:'addc',domain:domaine, type:'<?php echo $role; ?>'}).done(function(response){
        ...
        function delete_us(id)
        {
            ...
            if (result) {
                $.post('controller', {service:'deletecat',idg:id, type:'<?php echo $role; ?>'}).done(function(response){
                console.log(response)
                if(response=='done')
                {
                    user();
